/*
  # Create note templates and categories tables (Safe Migration)

  1. New Tables
    - `note_categories` - Stores clinic-specific note categories with colors and icons
    - `note_templates` - Stores clinic-specific note templates with JSONB structure
  
  2. Security
    - Enable RLS on both tables
    - Add clinic-based access policies
    - Users can only access their clinic's templates and categories
  
  3. Default Data
    - Seeds 8 default categories for each existing clinic
    - Seeds 3 default templates for each existing clinic
    - Includes SOAP, Follow-up, and Initial Consultation templates
  
  4. Performance
    - Indexes on clinic_id and is_active columns for fast queries
*/
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        AND policyname = 'Note categories: clinic access'
    ) THEN
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        AND policyname = 'Note templates: clinic access'
    ) THEN
...
-- Insert default categories for all existing clinics (only if they don't already exist)
DO $$
DECLARE
...
BEGIN
    ...
    FOR clinic_record IN SELECT id FROM public.clinics LOOP
        ...
        WHERE NOT EXISTS (
            ...
            WHERE clinic_id = clinic_record.id AND name = category_data.name
        );
        ...
        FROM (VALUES
            ...
            ('Initial Consultation', 'initial_visit', '{
              "sections": [
        ...
        WHERE NOT EXISTS (
            ...
            WHERE clinic_id = clinic_record.id AND name = template_data.name
        );
