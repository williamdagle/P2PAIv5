...
Deno.serve(async (req) => {
  console.log('🚀 get_supplements function invoked');
  ...
  try {
    ...
    const { data: supplements, error } = await query.order('start_date', {
      ascending: false
    });
    ...
    if (error) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to fetch supplements' }),
        {
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        }
    ...
    console.log('✅ Supplements fetched successfully');
    console.log('📈 Number of supplements returned:', supplements?.length || 0);
    console.log('💊 Supplements data preview:', supplements?.slice(0, 2));

    // Transform the data to include patient names
    console.log('🔄 Transforming supplement data...');
    ...
    return new Response(JSON.stringify(transformedSupplements), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error) {
    console.error('Error in get_supplements:', error);
