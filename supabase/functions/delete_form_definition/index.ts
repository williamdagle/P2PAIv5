...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!id) {
      return new Response(
        JSON.stringify({ error: 'Form definition ID is required' }),
    ...
    if (assignments && assignments.length > 0) {
      return new Response(
        JSON.stringify({ error: 'Cannot delete form with active assignments. Mark as inactive instead.' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    return new Response(
      JSON.stringify({ message: 'Form definition deleted successfully' }),
