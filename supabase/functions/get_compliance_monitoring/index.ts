...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!isAdmin) {
      return new Response(
        JSON.stringify({ error: 'Only admins can view compliance monitoring' }),
        {
          status: 403,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        }
    ...
    if (checksError) {
      console.error('Error fetching compliance monitoring:', checksError);
      return new Response(
        JSON.stringify({ error: 'Failed to fetch compliance monitoring' }),
        {
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        }
    ...
    console.error('Error in get_compliance_monitoring:', error);
    return new Response(
